...
function display(view, update) {
	...
	if (view == 'list') {
		...
		$((update) ? '.product-list > div[rel!=loaded]' : '.product-list > div').each(function(index, element) {
		...
		$((update) ? '.product-grid > div[rel!=loaded]' : '.product-grid > div').each(function(index, element) {
			html = '';
            html += '<div class="pbox">';				
			var image = $(element).find('.image').html();
