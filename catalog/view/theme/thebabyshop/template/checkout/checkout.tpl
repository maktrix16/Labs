...
<div id="content" class="span13"><?php echo $content_top; ?>
...
$('#changeshippingbox').live('click', function() {
	if ($('#changeshippingbox').attr('checked')=='checked') {
		$('#changeshippingbox').attr('checked','checked');
		$('#changeshippingbox').attr('value','T');
		...
		$('#changeshippingbox').attr('checked',false);
		$('#changeshippingbox').attr('value','F');
		$('#shipping-address').attr('style','display:normal');
		$('#shipping-address').slideDown('slow');

		$('#payment-method .checkout-heading').html('<?php echo $text_checkout_payment_method2;?>');
		$('#confirm .checkout-heading').html('<?php echo $text_checkout_confirm2;?>');
	}
});
//--></script> 
	...
	<?php }else{ ?>
		...
		*/
		$.webox({
			...
			width:370,
			bgvisibel:true,
			title:'<span class="wait">&nbsp;<img src="catalog/view/theme/default/image/loading.gif" alt="" /></span>',
			iframe:'index.php?route=account/login_pop'
		});
	<?php } ?>
});		
<?php } else { ?>
		...
		success: function(json) {
			...
			if (json['redirect']) {
				...
				if (json['error']['company']) {
					$('#payment-address input[name=\'company\'] + br').after('<span class="error">' + json['error']['telephone'] + '</span>');
				}	
...
$('#button-payment-address').live('click', function() {
	$.ajax({
		...
		type: 'post',
		data: $('#payment-address input[type=\'text\'], #payment-address input[type=\'password\'], #payment-address input[type=\'checkbox\'], #payment-address input[type=\'radio\']:checked, #payment-address input[type=\'hidden\'], #payment-address select'),
		dataType: 'json',
		beforeSend: function() {
		...
		success: function(json) {
			...
			if (json['redirect']) {
				...
				if (json['error']['telephone']) {
					$('#payment-address input[name=\'company\']').after('<span class="error">' + json['error']['telephone'] + '</span>');
				}		
	...
	$.ajax({
		...
		success: function(json) {
			...
			if (json['redirect']) {
				...
				if (json['error']['telephone']) {
					$('#shipping-address input[name=\'company\']').after('<span class="error">' + json['error']['telephone'] + '</span>');
				}		
