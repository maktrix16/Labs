...
class ModelCheckoutOrder extends Model {	
	...
	public function confirm($order_id, $order_status_id, $comment = '', $notify = false) {
		...
		if ($order_info && !$order_info['order_status_id']) {
			...
			$order_total_query_edi = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_total` WHERE order_id = '" . (int)$order_id . "' AND code='total' ORDER BY sort_order ASC");
			...
			if ($this->config->get('config_alert_mail')) {
				$subject = sprintf($language->get('text_new_subject'), html_entity_decode($this->config->get('config_name'), ENT_QUOTES, 'UTF-8'), $order_id);
                                ...
                                // Text
                                        $text = $order_id . "\t";
                                        $text .= "The Baby Shop\t\t";
                                        $text .= $order_info['shipping_firstname'] . " ";
                                        $text .= $order_info['shipping_lastname'] . "\t";
                                        $text .= $order_info['shipping_address_1'] . "\t";
                                        $text .= $order_info['shipping_address_2'] . "\t";
                                        $text .= $order_info['shipping_city'] . "\t";
                                        $text .= $order_info['shipping_zone'] . "\t";
                                        $text .= $order_info['shipping_zone_code'] . "\t";
                                        $text .= $order_info['shipping_country'] . "\t" ;
                                        $text .= $order_info['telephone'] . "\t" ;
                                        $text .= $order_info['email'] . "\t" ;
                                        $text .= $order_info['currency_code'] . "\t" ;
                                
                                foreach ($order_product_query->rows as $product) {
                                        foreach ($order_total_query_edi->rows as $total) {
                                            $text .= html_entity_decode($total['text'], ENT_NOQUOTES, 'UTF-8') . "\t\t\t\t\t" ;
                                        }

                                        $text .= $product['quantity'] . "\t" . $product['model'] . " ";
                                        $order_option_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_option WHERE order_id = '" . (int)$order_id . "' AND order_product_id = '" . $product['order_product_id'] . "'");
